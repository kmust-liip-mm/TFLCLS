...
class PeftModelForSeq2SeqLM(PeftModel):
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        ...
        if model_kwargs["past_key_values"] is None and self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            model_kwargs["past_key_values"] = past_key_values
        if isinstance(self.peft_config, PromptLearningConfig):
            if self.peft_config.peft_type == PeftType.PROMPT_TUNING and args[0][0][-1]==self.config.forced_bos_token_id:
                prompts = self.get_prompt(batch_size=model_kwargs["decoder_input_ids"].shape[0])
                model_kwargs["decoder_inputs_embeds"] = torch.cat(
                    (self.word_embeddings(model_kwargs["decoder_input_ids"]), prompts[:,20:,:]), dim=1
                )
